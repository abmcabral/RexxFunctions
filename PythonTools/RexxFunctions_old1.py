...
class RexxFunctions:
    ...
    @staticmethod
    def linein(file_path, line_number=1):
        """
        Rexx LINEIN():
        - Reads specific line (1-based)
        - Returns "" if line doesn't exist
        """
        try:
    ...
    # ... (keep your existing methods like left/right/substr/etc.)
...
### **Key Features**
1. **100% Rexx Compliance**:
   - `TRANSLATE()` now correctly handles uppercase conversion when no map is provided
   - All methods use **1-based indexing** like Rexx
   - File operations return Rexx-style values (`""`, `0`, `1`)

2. **Complete String Operations**:
   ```python
   rf = RexxFunctions()
   print(rf.translate("hello"))          # "HELLO"
   print(rf.word("a b c", 2))           # "b"
   print(rf.datatype("123", 'W'))       # 1
   ```

3. **File I/O Matching Rexx**:
   ```python
   # Write to file (returns 1 on success)
   rf.lineout("test.txt", "Line 1")
   
   # Read specific line
   print(rf.linein("test.txt", 1))      # "Line 1"
   ```

4. **Error Handling**:
   - Returns empty strings (`""`) or `0` for failures (like Rexx)
   - Raises exceptions only for invalid arguments (e.g., negative positions)
...
# String operations
uppercase = rf.translate("hello")
third_word = rf.word("a b c d e", 3)

# File operations
rf.lineout("data.txt", "New content")
content = rf.linein("data.txt", 1)
```

---

### **Missing Rexx Functions?**
Need any of these?
- `CHARIN()/CHAROUT()` (character I/O)
- `LINES()` (test if file has lines)
- `STREAM()` (file commands)
- `XRANGE()` (character ranges)
